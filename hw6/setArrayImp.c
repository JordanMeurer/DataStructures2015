...
setADT setDifference(setADT A, setADT B)
{
	...
	for(i = 0; i < B->length; i++){
		...
		if(unique == 1){
			Difference->array[Difference->length] = B->array[i];
			Difference->length++;
		}
	}
	for(i = 0; i < Difference->length; i++){
		for(j = 0; j < Difference->length - 1; j++){
			if(Difference->array[j] > Difference->array[j+1]){
				temp = Difference->array[j];
				Difference->array[j] = Difference->array[j+1];
				Difference->array[j+1] = temp;
			}
		}
	}
	return(Difference);
